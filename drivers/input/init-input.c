...
static int ctp_fetch_sysconfig_para(enum input_sensor_type *ctp_type)
{
	...
	type = script_get_item("tp_light", "tp_light_gpio", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		printk("script_get_item tp_light_gpio err\n");
		goto script_get_item_err;
	}
...
{	
	...
	if (data->ctp_power) {
		...
		if (!data->ctp_power_ldo)
			printk("%s: could not get ctp ldo '%s' , check"
		...
		if(0 != gpio_request(data->ctp_power_io.gpio, NULL))
			printk("ctp_power_io gpio_request is failed,"
